...
def _row_to_kiosk(r) -> dict:
    ...
    return {
        ...
        "smokeWarning": smoke,
        "cameraImageUrl": r["camera_snapshot_url"],
...
def update_kiosk_name(kiosk_id: str, name: str) -> Optional[dict]:
    with db() as conn:
        conn.execute("UPDATE kiosks SET name=?, updated_at=? WHERE id=? AND is_active=1", (name, now_utc_iso(), kiosk_id))
    return get_kiosk(kiosk_id)

def delete_kiosk(kiosk_id: str) -> bool:
    with db() as conn:
        cur = conn.execute("UPDATE kiosks SET is_active=0, updated_at=? WHERE id=? AND is_active=1", (now_utc_iso(), kiosk_id))
        return cur.rowcount > 0

def update_telemetry(kiosk_id: str, temperature_c: float | None, humidity_pct: float | None, smoke: bool | None, camera_url: str | None) -> Optional[dict]:
    with db() as conn:
        r = conn.execute("SELECT id FROM kiosks WHERE id=? AND is_active=1", (kiosk_id,)).fetchone()
        if not r:
            return None
        existing = conn.execute("SELECT temperature_c, humidity_pct, smoke, camera_snapshot_url FROM kiosks WHERE id=?", (kiosk_id,)).fetchone()
        ...
        cam = existing["camera_snapshot_url"] if camera_url is None else camera_url
        conn.execute(
            "UPDATE kiosks SET temperature_c=?, humidity_pct=?, smoke=?, camera_snapshot_url=?, updated_at=? WHERE id=?",
            (temp, hum, sm, cam, now_utc_iso(), kiosk_id),
        )
    return get_kiosk(kiosk_id)
